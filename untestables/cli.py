...
@click.option('--rescan-days', type=int,
              help='Re-scan repositories that were last scanned more than this many days ago')
@click.option('--force-rescan', is_flag=True, help='Force re-scan of all repositories, ignoring last scan time')
@click.option('--end-time-iso', type=str, default=None,
              help='Optional ISO timestamp for when the find-repos process should stop itself.')
def find_repos(min_stars: int, max_stars: int, rescan_days: Optional[int] = None, force_rescan: bool = False,
               end_time_iso: Optional[str] = None) -> None:
    ...
    if end_time_iso:
        try:
            ...
            logger.error(
                f"Invalid --end-time format: '{end_time_iso}'. Please use ISO format. Continuing without time limit.")
    ...
    try:
        ...
        for repo in repos:
            ...
            if end_time_dt and datetime.now(timezone.utc) >= end_time_dt:
                logger.info(
                    f"Time limit ({end_time_dt.isoformat()}) reached within find-repos loop. Stopping repository processing.")
                break  # Exit the loop
...
@cli.command('scan')
@click.option('--duration', default='7d',
              help='Total duration to run the scanner for (e.g., 7d, 12h, 30m). Default 7 days.')
@click.option('--no-gaps-sleep', default='1h',
              help='Sleep interval when no gaps are found (e.g., 1h, 30m). Default 1 hour.')
...
def scan(duration: str, no_gaps_sleep: str, cycle_sleep: str):
    ...
    logger.info(
        f"Starting continuous scan process. Duration: {duration}, No Gaps Sleep: {no_gaps_sleep}, Cycle Sleep: {cycle_sleep}")
    ...
    while datetime.now(timezone.utc) < end_time_overall:
        ...
        logger.info(
            f"Overall scan end time: {end_time_overall.isoformat()}. Remaining duration for this cycle and its subprocesses: {remaining_duration_for_cycle_td}")
        ...
        if analyzer.api_limit_reset_time and datetime.now(timezone.utc) < analyzer.api_limit_reset_time:
            ...
            logger.info(
                f"API limit encountered in cycle. Reset at {analyzer.api_limit_reset_time}. Sleeping for {wait_time_seconds:.0f}s.")
            time.sleep(wait_time_seconds)
        elif not scan_attempted:  # No gaps found, and not an API limit known to analyzer before starting cycle
            logger.info(
                f"No gaps found or scan not attempted for other reasons (e.g. proactive API limit check). Sleeping for {no_gaps_sleep_seconds:.0f}s (no_gaps_sleep).")
